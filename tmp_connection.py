...
class MainWindow(QMainWindow):
    ...
    # Initialization aplication window
    ...
    # Function to set SQL parameters and dynamic interface working
    ...
    def combobox_age_struct_list(self):
        ...
        # оптимизировано все что выше
        self.dict_sql_settings['current_age_struct_type_data_list'] = fc.get_list_from_sql(self.dict_sql_settings,
                             f'SELECT DISTINCT [{self.ui.comboBox_tab_age_struct_type_list_choose_column.currentText()}] '
                             f'FROM [{self.ui.comboBox_tab_sql_bioanalis_table_list.currentText()}]'
                             )
        self.dict_sql_settings['current_age_struct_year_data_list'] = fc.get_list_from_sql(self.dict_sql_settings,
                             f'SELECT DISTINCT [{self.ui.comboBox_tab_age_struct_year_list_choose_column.currentText()}] '
                             f'FROM [{self.ui.comboBox_tab_sql_bioanalis_table_list.currentText()}]'
                             )
        self.dict_sql_settings['current_age_struct_area_data_list'] = fc.get_list_from_sql(self.dict_sql_settings,
                             f'SELECT DISTINCT [{self.ui.comboBox_tab_age_struct_area_list_choose_column.currentText()}] '
                             f'FROM [{self.ui.comboBox_tab_sql_bioanalis_table_list.currentText()}]'
                             )
